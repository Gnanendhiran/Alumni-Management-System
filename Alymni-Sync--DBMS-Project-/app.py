...
@login_required
def feed():
    """Display the social networking feed and sidebar stats."""
    user_id = session.get('user_id')
    role = session.get('role')
    
    # Fetch current user profile info for the sidebar
    if role == 'alumni':
        user_profile = execute_query("SELECT * FROM alumni_profiles WHERE user_id = %s AND is_deleted = 0", (user_id,))
    else:
        user_profile = execute_query("SELECT * FROM student_profiles WHERE user_id = %s AND is_deleted = 0", (user_id,))
    
    # Fetch user counts for sidebar
    posts_count = execute_query("SELECT COUNT(*) as cnt FROM posts WHERE user_id = %s AND is_deleted = 0", (user_id,))[0]['cnt']
    followers_count = execute_query("SELECT COUNT(*) as cnt FROM follows WHERE following_id = %s", (user_id,))[0]['cnt']
    following_count = execute_query("SELECT COUNT(*) as cnt FROM follows WHERE follower_id = %s", (user_id,))[0]['cnt']
    ...
    # Fetch all active posts, including dynamic likes checking via SQL subquery
    posts = execute_query("""
        SELECT p.*, u.email,
               COALESCE(ap.first_name, sp.first_name) as first_name,
               COALESCE(ap.last_name, sp.last_name) as last_name,
               COALESCE(ap.profile_photo, sp.profile_photo) as profile_photo,
               CASE WHEN ap.id IS NOT NULL THEN 'alumni' ELSE 'student' END as author_role,
               COALESCE(ap.id, sp.id) as author_profile_id,
               (SELECT COUNT(*) FROM post_likes WHERE post_id = p.id AND user_id = %s) as user_has_liked
        FROM posts p
...
def create_post():
    ...
    user_id = session.get('user_id')
    execute_query("""
        INSERT INTO posts (user_id, content, created_by) 
        VALUES (%s, %s, %s)
    """, (user_id, content.strip(), user_id), fetch=False)
...
def like_post(post_id):
    """Toggle liking/unliking a social post."""
    user_id = session.get('user_id')
    
    # Check if already liked
    liked = execute_query("SELECT id FROM post_likes WHERE post_id = %s AND user_id = %s", (post_id, user_id))
    
    if liked:
        # Unlike (deletes like from db, database trigger handles count decrement)
        execute_query("DELETE FROM post_likes WHERE post_id = %s AND user_id = %s", (post_id, user_id), fetch=False)
    else:
        # Like (inserts like to db, database trigger handles count increment)
        execute_query("INSERT INTO post_likes (post_id, user_id) VALUES (%s, %s)", (post_id, user_id), fetch=False)
...
def comment_post(post_id):
    ...
    user_id = session.get('user_id')
    execute_query("""
        INSERT INTO post_comments (post_id, user_id, content, created_by) 
        VALUES (%s, %s, %s, %s)
    """, (post_id, user_id, content.strip(), user_id), fetch=False)
...
def follow_user(user_id):
    ...
    following = execute_query("SELECT id FROM follows WHERE follower_id = %s AND following_id = %s", (follower_id, user_id))
    
    if following:
        # Unfollow
        execute_query("DELETE FROM follows WHERE follower_id = %s AND following_id = %s", (follower_id, user_id), fetch=False)
        flash('Unfollowed successfully.', 'info')
    else:
        # Follow
        execute_query("INSERT INTO follows (follower_id, following_id) VALUES (%s, %s)", (follower_id, user_id), fetch=False)
        flash('Followed user successfully!', 'success')
